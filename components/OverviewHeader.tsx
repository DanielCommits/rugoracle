...
export function OverviewHeader({ address, riskScore, riskLevel }: OverviewHeaderProps) {
  ...
  return (
    <div className="bg-gradient-to-r from-slate-900/50 via-slate-900/50 to-slate-900/30 border border-slate-800 rounded-lg p-8 mb-8">
      {/* Address Section */}
      <div className="mb-8">
        <p className="text-sm text-slate-400 mb-2">Analyzing Address</p>
        <div className="flex items-center gap-3">
          <code className="text-slate-100 font-mono text-lg break-all">{address}</code>
          <button
            onClick={handleCopy}
      ...
      <div className="flex items-end justify-between gap-8">
        <div>
          ...
          <div className={`text-6xl font-bold ${getScoreColor(riskScore)}`}>{riskScore}</div>
          <p className="text-slate-400 text-sm mt-1">out of 100</p>
        </div>

        {/* Status Badge */}
        <div className={`px-6 py-3 rounded-lg border font-bold text-lg ${getStatusBg(riskLevel)}`}>
